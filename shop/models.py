...
class Basket(models.Model):
    ...
    def basket_total(self):
        total = 0
        if self.basket_item_list:
            total = sum([bi.item_total for bi in self.basket_item_list])
        return total
...
class Order(models.Model):
    ...
    @property
    def total(self):
        items = self.basket_item_objects.all()
        total = 0
        total = sum([bi.item_total for bi in items]) if items  else 0
        return total
